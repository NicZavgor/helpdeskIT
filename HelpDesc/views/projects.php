...
<div class="mb-3" style="padding-top: 20px;padding-left: 10%;  padding-right: 10%;">
    <div class="row g-7">
        <div class="col-sm-10">
            <form class="form-group row" action="?page=projects"  method="post" name="Search" id="Search">
                <h3  class="col-sm-2">Проекты</h3>
                <div class="col-sm-9">
                ...
                <input type="submit" name="ok" class="btn btn-outline-secondary" value="Найти" onclick="doSearch(); return false;">
                         ...
                         <div class="col-sm-2">
                              <button class="btn btn-primary" value="Создать" data-bs-toggle="modal" data-bs-target="#editModal" onclick="location.href='?page=projectedit&action=new'">Создать </button>
                         </div>
    </div>
    ...
    <table  class="table" >
        ...
        <?php foreach ($Projects as $One): ?>
            <tr  id="row-<?= $One['id'] ?>">
                ...
                <td>
                    <?= (new DateTime($One['start_date']))->format('Y.m.d')?>
                </td>
                <td>
                    <?= (new DateTime($One['end_date']))->format('Y.m.d')?>
                </td>
                <td>
                     <a href="?page=project&id=<?=$One['id']?>" class="btn btn-outline-primary">...</a>
    ...
    <nav aria-label="...">
        ...
        <?php else: ?>
            <li class="page-item"> <a class="page-link" href="?page=projects&pg=prior&curpg=<?=$newpg?>">Предыдущая</a> </li>
        <?php endif;?>
        <?php if($newpg>1):?>
            <li class="page-item"><a class="page-link" href="?page=projects&pg=<?=$newpg-1?>&curpg=<?=$newpg?>"><?=$newpg-1?></a></li>
        <?php endif;?>
        ...
        <?php else: ?>
            <li class="page-item"> <a class="page-link" href="?page=projects&pg=next&curpg=<?=$newpg?>">Следующая</a> </li>
        <?php endif;?>
        </ul>
    </nav>
...
<?php endif;?>
    ...
    async function deleteOne($id) {
        ...
        try {
             const response = await fetch('?page=projects&action=delete', {
                                 method: 'POST',
    ...
    async function saveChanges() {
        ...
        try {
             const response = await fetch('?page=projects&api&action=edit', {
                                 method: 'POST',
                                 body: JSON.stringify({id: $id,
                                    name: $name,
                                    itrole: $ItRole,
